...
import { MAP_IDS, pickMapId } from "../../shared/mapManifest";
...
export function normalizeMapId(value: unknown, fallback: number): number {
  const mapId = typeof value === "number" ? value : Number(value);
  return Number.isInteger(mapId) && VALID_MAP_IDS.has(mapId) ? mapId : fallback;
}
...
export async function setPostMap(postId: string, mapId: number): Promise<void> {
  ...
  await redis.set(
    postMapKey(postId),
    String(normalizeMapId(mapId, daily.mapId)),
  );
}
...
export async function getPostLevelInfo(
  ...
  if (stored) {
    return { daily, mapId: normalizeMapId(stored, daily.mapId) };
  }
